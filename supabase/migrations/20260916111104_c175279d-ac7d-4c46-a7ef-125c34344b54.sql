CREATE INDEX IF NOT EXISTS idx_practice_attempts_day_completed ON public.practice_attempts (local_day_key) WHERE completed_at IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_interview_attempts_day_completed ON public.interview_attempts (local_day_key) WHERE completed_at IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_story_views_last_opened ON public.story_episode_views (last_opened_at);
CREATE INDEX IF NOT EXISTS idx_story_views_first_opened ON public.story_episode_views (first_opened_at);
...
begin
  ...
  with events as (
    ...
    union all
    -- El Mundo de Vale: count each distinct local day a view row touched
    select v.d, v.user_id, 'story', 1
      from (
        select distinct sev.user_id,
               ((sev.first_opened_at at time zone _tz)::date) as d
          from public.story_episode_views sev
        union
        select distinct sev.user_id,
               ((sev.last_opened_at at time zone _tz)::date) as d
          from public.story_episode_views sev
      ) v
