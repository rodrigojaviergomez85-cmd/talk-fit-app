...
BEGIN
  ...
  IF my_points IS NULL THEN
    -- Not a member: only admin / unlimited accounts may watch another cohort.
    IF NOT public.league_is_excluded(uid) THEN RETURN jsonb_build_object('enrolled', false); END IF;
...
REVOKE ALL ON FUNCTION public.league_summary_for_cohort(text, integer) FROM PUBLIC;
