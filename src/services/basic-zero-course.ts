...
/** One angle = one Rep 4 prompt topic. Its position in the ORDER list fixes the `${id}-p<n>` id. */
type Angle = [cue: string, question: string, questionEs: string, starter: string, starterEs: string];

/**
 * Picks the 3 angles for a given day (1 familiar + 2 rotated from the day's modeled
 * content). Rep 4 shows max 3 prompts, so every day exposes a different set while the
 * week still covers all 8 topics.
 */
...
const SELF_FOUNDATION_ORDER = ["name", "age", "country", "city", "color", "food", "hobbies", "personality", "nameAge", "countryCity", "hobbiesPersonality"];
const SELF_FOUNDATION: Record<string, Angle> = {
  ...
  // Day 5 challenge: two ideas in one answer.
  ...
  hobbiesPersonality: ["WHAT", "What are your hobbies and how would you describe yourself?", "¿Cuáles son tus pasatiempos y cómo te describirías?", "My hobbies are ______ and ______. Overall, I am a ______ person.", "Mis pasatiempos son ______ y ______. En general, soy una persona ______."],
};
const SELF_FOUNDATION_BY_DAY: Record<number, string[]> = {
  1: ["name", "age", "country"],
  2: ["city", "color", "food"],
  3: ["hobbies", "city", "color"],
  4: ["food", "hobbies", "personality"],
  5: ["nameAge", "countryCity", "hobbiesPersonality"],
};
...
const SELF_FLUENCY_ORDER = ["nameAge", "countryCity", "colorWhy", "foodWhy", "hobbies", "freeTime", "also", "personality", "aboutYou", "favoritesWhy", "freeTimePersonality"];
const SELF_FLUENCY: Record<string, Angle> = {
  ...
  personality: ["HOW", "How would you describe yourself?", "¿Cómo te describirías?", "Overall, I am a ______ and ______ person.", "En general, soy una persona ______ y ______."],
  // Day 10 challenge: connected answers.
  aboutYou: ["WHAT", "Tell me about yourself. What is your name, and where are you from?", "Háblame de ti. ¿Cómo te llamas y de dónde eres?", "My name is ______, I am ______ years old, and I am from ______.", "Me llamo ______, tengo ______ años y soy de ______."],
  favoritesWhy: ["WHY", "What are your favorite color and food, and why?", "¿Cuáles son tu color y tu comida favoritos, y por qué?", "My favorite color is ______ because… My favorite food is ______ because…", "Mi color favorito es ______ porque… Mi comida favorita es ______ porque…"],
  freeTimePersonality: ["HOW", "What do you like to do in your free time, and how would you describe yourself?", "¿Qué te gusta hacer en tu tiempo libre y cómo te describirías?", "In my free time, I like to… Overall, I am a ______ and ______ person.", "En mi tiempo libre me gusta… En general, soy una persona ______ y ______."],
};
const SELF_FLUENCY_BY_DAY: Record<number, string[]> = {
  6: ["nameAge", "countryCity", "colorWhy"],
  7: ["foodWhy", "hobbies", "freeTime"],
  8: ["countryCity", "freeTime", "also"],
  9: ["colorWhy", "foodWhy", "personality"],
  10: ["aboutYou", "favoritesWhy", "freeTimePersonality"],
};
...
const OTHER_FOUNDATION_ORDER = ["who", "name", "age", "country", "city", "color", "food", "hobbies", "whoName", "countryCity", "foodHobbies"];
const OTHER_FOUNDATION: Record<string, Angle> = {
  ...
  // Day 15 challenge: two ideas in one answer.
  whoName: ["WHO", "Who is this person, and what is his / her name?", "¿Quién es y cómo se llama?", "This is my ______. His/Her name is ______.", "Esta es mi ______ / Este es mi ______. Su nombre es ______."],
  countryCity: ["WHERE", "Where is he / she from, and where does he / she live?", "¿De dónde es y dónde vive?", "He/She is from ______. He/She lives in ______.", "Es de ______. Vive en ______."],
  foodHobbies: ["WHAT", "What is his / her favorite food, and what are his / her hobbies?", "¿Cuál es su comida favorita y cuáles son sus pasatiempos?", "His/Her favorite food is ______. His/Her hobbies are ______ and ______.", "Su comida favorita es ______. Sus pasatiempos son ______ y ______."],
};
const OTHER_FOUNDATION_BY_DAY: Record<number, string[]> = {
  11: ["who", "name", "age"],
  12: ["country", "city", "color"],
  13: ["name", "food", "hobbies"],
  14: ["who", "city", "hobbies"],
  15: ["whoName", "countryCity", "foodHobbies"],
};
...
const OTHER_FLUENCY_ORDER = ["whoName", "ageCountry", "city", "colorWhy", "foodWhy", "hobbies", "freeTime", "personality", "who"];
const OTHER_FLUENCY: Record<string, Angle> = {
  ...
  who: ["WHO", "Who is this person?", "¿Quién es esta persona?", "This is my ______, and he/she is very important to me.", "Esta es mi ______ / Este es mi ______ y es muy importante para mí."],
};
const OTHER_FLUENCY_BY_DAY: Record<number, string[]> = {
  16: ["whoName", "ageCountry", "colorWhy"],
  17: ["foodWhy", "hobbies", "freeTime"],
  18: ["city", "colorWhy", "personality"],
  19: ["who", "freeTime", "personality"],
  20: ["whoName", "foodWhy", "personality"],
};
...
const carlos: Person = {
  ...
  foodWhyOther: "he loves cheese", foodWhyOtherEs: "le encanta el queso",
  ...
  freeOther: "play soccer with his friends", freeOtherEs: "jugar fútbol con sus amigos",
...
const sofia: Person = {
  ...
  colorWhyOther: "it makes her happy", colorWhyOtherEs: "la hace feliz",
  food: "tacos", foodEs: "los tacos", foodWhy: "they remind me of my family", foodWhyEs: "me recuerdan a mi familia",
  foodWhyOther: "they remind her of her family", foodWhyOtherEs: "le recuerdan a su familia",
...
const daniel: Person = {
  ...
  colorWhyOther: "it gives him energy", colorWhyOtherEs: "le da energía",
...
function week1Day(day: number, p: Person, topic: string, topicEs: string): CourseDay {
  ...
  return makeDay({
    ...
    prompts: selfPromptsFoundation(id, day),
    cues: SELF_CUES,
...
function week2Day(day: number, p: Person, topic: string, topicEs: string): CourseDay {
  ...
  return makeDay({
    ...
    prompts: selfPromptsFluency(id, day),
    cues: SELF_CUES,
...
function week3Day(day: number, p: Person, topic: string, topicEs: string): CourseDay {
  ...
  return makeDay({
    ...
    prompts: otherPromptsFoundation(id, day),
    cues: OTHER_CUES,
...
function week4Day(day: number, p: Person, topic: string, topicEs: string): CourseDay {
  ...
  return makeDay({
    ...
    prompts: otherPromptsFluency(id, day),
