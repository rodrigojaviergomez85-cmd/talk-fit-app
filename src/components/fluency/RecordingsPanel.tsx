...
/**
 * Audio half of PROGRESS ("am I improving?" answered with the learner's own voice).
 * Previously the standalone /recordings screen; behaviour is unchanged.
 */
export function RecordingsPanel({ state }: { state: JourneyState }) {
  ...
  const marks = useMemo(() => milestones(state), [state]);
  const total = JourneyService.completedCount(state);
  ...
  return (
    <div className="space-y-6">
      {total === 0 ? (
        ...
        <>
          ...
          <section className="space-y-3">
            ...
            <RecordingsGrouped state={state} onCompare={setSheet} />
          </section>
        </>
      )}

      {/* Coach Check — secondary action; audio comes first on this tab */}
      <Link
        to="/coach-check"
      ...
      <MomentSheet comparison={sheet} state={state} onClose={() => setSheet(null)} />
    </div>
  );
}
