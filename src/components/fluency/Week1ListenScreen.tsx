    ...
    <header className="border-b border-border bg-listen-surface px-4 pb-4 pt-[max(0.65rem,env(safe-area-inset-top))]">
      <div className="relative mx-auto w-full max-w-[480px]">
        <div className="flex min-h-11 items-center justify-between">
          <Button variant="ghost" size="icon" onClick={onBack} aria-label={t("listen.back")} className="size-11 rounded-full">
        ...
        <div className="mt-2 flex items-center justify-between gap-3">
          <strong className="text-[16px] text-foreground">{t("listen.day").replace("{day}", String(day.day))}</strong>
          <span className="text-right text-[14px] font-semibold text-muted-foreground">{t("listen.step")}</span>
        </div>
        <div className="mt-3 grid grid-cols-5 gap-2" aria-label={t("listen.stepsAria")}>
...
function ChallengeCard({ day }: { day: CourseDay }) {
  ...
  return (
    <section className="rounded-[22px] bg-navy p-5 text-navy-foreground shadow-[var(--shadow-card)]">
      <p className="text-[11px] font-extrabold uppercase tracking-[0.18em] text-primary">{t("listen.challenge")}</p>
      ...
      <div className="mt-4 grid grid-cols-2 gap-2">
        {!spanishAll ? (
          ...
          >
            <Languages /> {translated ? t("listen.hideTranslation") : t("listen.translate")}
          </Button>
        ) : <span />}
        <AudioPlayer
    ...
    <div className="min-h-screen bg-listen-surface pb-[max(2rem,env(safe-area-inset-bottom))]">
      ...
      <main className="mx-auto w-full max-w-[480px] space-y-5 px-4 py-6 min-[380px]:px-5">
        <div>
          ...
          <p className="mt-4 text-[17px] font-medium text-muted-foreground">{t("listen.lead")}</p>
          <p className="mt-1 text-[12px] text-muted-foreground">{t("listen.hint")}</p>
        </div>
        ...
        <section className="rounded-[22px] border border-primary/10 bg-listen-player p-5 shadow-[var(--shadow-card)]">
          <div className="flex items-center gap-4">
            {speaker?.avatarSrc && !avatarFailed ? (
              <img
            ...
            <div className="min-w-0">
              <h2 className="text-[20px] font-extrabold text-foreground">
                {speaker?.name
                  ? t("listen.speakerName").replace("{name}", speaker.name)
                  : t("listen.speakerFallback")}
              </h2>
              <p className="text-[14px] text-muted-foreground">{t("listen.exampleAnswer")}</p>
            </div>
          </div>
          ...
          <div className="mt-2 flex items-center gap-3 text-[12px] font-bold tabular-nums text-muted-foreground">
            ...
            <div className="h-1.5 flex-1 overflow-hidden rounded-full bg-muted" role="progressbar" aria-label={t("listen.progressAria")} aria-valuemin={0} aria-valuemax={100} aria-valuenow={Math.round(progress)}>
          ...
          <AudioPlayer
            ...
            onStatusChange={(status) => {
              ...
              // An interrupted or restarted clip clears the old position, so the
              // timeline never shows progress the service can no longer resume.
          ...
          <Button variant="ghost" onClick={() => setShowText((open) => !open)} aria-expanded={showText} className="mt-3 min-h-11 w-full text-muted-foreground">
            <BookOpenText /> {showText ? t("listen.hideText") : t("listen.showText")}
          </Button>
        </section>

        {showText ? (
          <section className="space-y-3 rounded-[22px] border border-border bg-card p-5 shadow-[var(--shadow-card)]" aria-label={t("listen.textAria")}>
            {day.lines.map((line) => (
        ...
        <div className="space-y-2 pt-1">
          <Button
            onClick={onNext}
            disabled={!heard}
            className="min-h-14 w-full rounded-2xl bg-primary text-[16px] font-extrabold text-navy hover:bg-primary hover:brightness-105 disabled:bg-muted disabled:text-muted-foreground disabled:opacity-100"
          >
            {t("listen.continue")} <ArrowRight />
          </Button>
          {!heard ? <p className="text-center text-[12px] font-medium text-muted-foreground">{t("listen.skipHint")}</p> : null}
          <Button variant="link" onClick={skip} className="min-h-11 w-full text-muted-foreground">
            {t("listen.skip")}
          </Button>
          <span className="sr-only" aria-live="polite">
            {audioStatus === "playing"
              ? t("listen.announcePlaying")
              : audioStatus === "paused"
                ? t("listen.announcePaused")
                : audioStatus === "ended"
                  ? t("listen.announceEnded")
                  : ""}
          </span>
        </div>
      </main>
