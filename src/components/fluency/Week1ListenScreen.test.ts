...
import { BASIC_ZERO_COURSE } from "@/services/basic-zero-course";
...
describe("Basic Zero Week 1 listening pilot", () => {
  it("activates only for Basic Zero Week 1 Step 1", () => {
    const week1 = BASIC_ZERO_COURSE[0] as CourseDay;
    const week2 = BASIC_ZERO_COURSE[5] as CourseDay;
  ...
  it("uses the authored speaker and voice for Days 1–5", () => {
    expect(BASIC_ZERO_COURSE.slice(0, 5).map((day) => [day.speaker?.name, day.speakerVoice])).toEqual([
      ["Carlos", "male"],
      ["Sofia", "female"],
      ["Daniel", "male"],
      ["Valeria", "female"],
