...
BEGIN
  ...
  UPDATE auth.users SET
    ...
    -- Keep email_confirmed_at so login attempts return invalid_credentials instead of
    -- email_not_confirmed; never mark the user as banned because that reveals the
  ...
  -- Remove any MFA setup so no alternative auth path remains.
  DELETE FROM auth.mfa_challenges WHERE factor_id IN (
    SELECT id FROM auth.mfa_factors WHERE user_id = _user_id
  );
...
-- Backfill existing anonymized accounts so they no longer appear banned.
UPDATE auth.users SET
  banned_until = NULL,
...
WHERE email LIKE 'usuario-eliminado-%@deleted.invalid';

DELETE FROM auth.identities
WHERE user_id IN (
  SELECT id FROM auth.users WHERE email LIKE 'usuario-eliminado-%@deleted.invalid'
);
